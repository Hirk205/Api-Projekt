    ...
    if ($this->method== 'GET'){
        ...
        if($this->params){
            ...
            if(count($this->params)>1){
                ...
                if($count>0){
                    $allowArray=array("iduser","hoten","mail","username","password","phone","address","dob","level");
                    $sql="select ";
                    for ($i=1; $i<=$count;$i++){
                        if(!in_array($fields[$i-1],$allowArray))
                            return $this->response(405, "fields not allow");
        ...
        if(strlen($obj['username'])<6){
            return $this->response(404, "username length must above or equal 6"); 
        }
        if(strlen($obj['password'])<6){
            return $this->response(404, "password length must above or equal 6"); 
        }
        $data=0;
        if($this->params){
            ...
            if(isset($_GET['action'])){
                ...
                if($db->NumRows($sql)){
                    ...
                    if(password_verify($obj['password'],$hashed_password )){
                        ...
                        return $this->response(404, "password wrong");
                    }
                }
                else{
                ...
                if($db->NumRows($sql)){
                    ...
                    $sql="INSERT INTO user (iduser,username,hoten,password,mail,phone,address,dob,level)
                        VALUES (NULL,'$obj[username]','$obj[hoten]','$password','$obj[mail]','$obj[phone]','$obj[address]','$date','0')";
        ...
        if($this->params){
            ...
            $getData=$db->FetchAll($sql);
            foreach($getData as $i){
                $sql="delete from chitiethoadon where idbill='$i'";
                $db->ExecuteQuery($sql);
            }
